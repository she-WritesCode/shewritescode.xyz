...
export const NavItem: React.FC<NavItemProps> = ({ children, icon, href }) => {
    return <Link href={href} >
        <motion.div initial={{ y: 0 }} whileHover={{ y: -7 }} className="relative flex items-center gap-4 p-2 md:pr-4 rounded-xl">
            ...
            <div className="text-xl hidden md:block">{children}</div>
        </motion.div>
    </Link>
}